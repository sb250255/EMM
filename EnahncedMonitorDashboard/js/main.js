			...
			datasets: [
				{
					...
					label: 'No Transactions past 6 hours (In Count)'
				}]
		};
...
HMZAdminMgr.renderNewAccountsChart1 = function (details) {
    var ctx = document.getElementById('newAccountsChart1');
    var data = {
        labels: details.labels,
        datasets: [
            {
                ...
                label: 'Withdrawal Unavailable (InCount)'
            }]
    };
    var barChart = new Chart(ctx, {
        type: 'horizontalBar',
...
HMZAdminMgr.rebootReport = function (details) {

    var ctx = document.getElementById('rebootReason');
    var data = {
        labels: details.labels,
        datasets: [
            {
    ...
    $('#rebootReason').closest('table').find(".chartData").html(HMZAdminMgr.generateTabularData(data));
}
...
HMZAdminMgr.generateTabularData = function(data){
	...
	for(var i=0,len=labels.length;i<len;i++){
	    tableHtml += '<tr><td style="cursor:pointer" ng-click="getOOSData()"><i style="color:' + color[i] + '" class="icon square"></i>' + labels[i] + '</td><td>' + dataset[i] + '</td></tr>';
	}
	tableHtml += "</table>";
	return tableHtml;
